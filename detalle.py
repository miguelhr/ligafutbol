...
tarjerojajugavisi = resp_xml.xpath("/match/lineups/visitor/reds/text()")
goljajugavisi = resp_xml.xpath("/match/lineups/visitor/goals/text()")
tarjeamarillajugavisi = resp_xml.xpath("/match/lineups/visitor/yellows/text()")
tarjerojajugalocal = resp_xml.xpath("/match/lineups/local/reds/text()")
goljajugalocal = resp_xml.xpath("/match/lineups/local/goals/text()")
tarjeamarillajugalocal = resp_xml.xpath("/match/lineups/local/yellows/text()")
...
for i in range(numerojugadoreslocal):
    ...
    imgElt1 = etree.SubElement(tdElt1, "img", src="%s" % (fotojugalocal[i]), alt="local", title="%s Goles %s T.Rojas %s T.Amarillas" %(goljajugalocal[i].replace("\n",''),tarjerojajugalocal[i],tarjeamarillajugalocal[i]))
...
for i in range(contador):
    ...
    imgElt20 = etree.SubElement(tdElt23, "img", src="%s" % (escudolocal[i]), alt="fotolocal", title="%s" %(twitterlocal[i].replace("\n",'')))
    tdElt24 = etree.SubElement(trElt22, "td")
    tdElt24.text = "%s" % (resultado[i])
    tdElt25 = etree.SubElement(trElt22, "td")
    imgElt21 = etree.SubElement(tdElt25, "img", src="%s" % (escudovisitante[i]), alt="fotovisi", title="%s" %(twittervisi[i].replace("\n",'')))
...
for i in range(numerojugadoresvisi):
    ...
    imgElt10 = etree.SubElement(tdElt10, "img", src="%s" % (fotojugavisi[i]), alt="visitante", title="%s Goles %s T.Rojas %s T.Amarillas" %(goljajugavisi[i].replace("\n",''),tarjerojajugavisi[i],tarjeamarillajugavisi[i]))
